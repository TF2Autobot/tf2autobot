...
export default class PricelistManagerCommands {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (params.sku === undefined) {
        if (params.item !== undefined) {
          ...
          if ((params.sku as string).includes('null') || (params.sku as string).includes('undefined')) {
            errorMessage.push(
              `❌ Failed to add ${params.sku as string}: The sku for "${params.item as string
      ...
      if (params.promoted !== undefined) {
        if (!isPremium) {
          errorMessage.push(
            `❌ Failed to add ${this.bot.schema.getName(SKU.fromString(params.sku as string))} (${params.sku as string
            }): This account is not Backpack.tf Premium. You can't use "promoted" parameter.`
          );
          failed++;
          continue;
      ...
      try {
        ...
        for (let i = 0; i < count2; i++) {
          ...
          this.bot.pricelist
            ...
            .catch(err => {
              errorMessage.push(
                `❌ Error adding ${this.bot.schema.getName(SKU.fromString(entry.params.sku))} (${entry.params.sku
  ...
  autoAddCommand(steamID: SteamID, message: string): void {
    ...
    this.bot.sendMessage(
      ...
      `\n${params.autoprice ? 2 : 1} seconds in between items, so it will be about ${totalTime < aMin
  ...
  async updateCommand(steamID: SteamID, message: string): Promise<void> {
    ...
    if (params.all === true) {
      ...
      if (params.withintent) {
        if (typeof params.note == 'object') {
          return this.bot.sendMessage(
            steamID,
            `❌ Please edit note settings using "!config".` +
            `\nExample: "!config details.<buy/sell>=<yourNote>"`
          );
        }
      }
      ...
      for (const sku in pricelist) {
        ...
        if (params.withgroup || params.withoutgroup || params.withintent) {
          if (typeof params.note === 'object' && !params.withintent) {
      ...
      if (changed) {
        ...
        if (params.withgroup) {
          ...
          return this.bot.sendMessage(
            steamID,
            `❌ There is no entry other than "${params.withoutgroup as string
        ...
        if (params.withintent) {
          return this.bot.sendMessage(
            steamID,
            `❌ There is no entry with "${params.withintent as string}" intent found in your pricelist.`
          );
        }
      }
    ...
    const itemEntry = this.bot.pricelist.getPrice({
      priceKey,
      onlyEnabled: false
    });
    ...
    const entryData = this.bot.pricelist
      .getPrice({
        priceKey,
        onlyEnabled: false
      })
      .getJSON(); //TODO: CONTINUE
  ...
  async updatebulkCommand(steamID: SteamID, message: string): Promise<void> {
    ...
    if (isHasAutoprice) {
      try {
        ...
        return this.bot.sendMessage(
          steamID,
          `❌ Bulk update operation aborted: Failed to obtain pricelist from pricer: ${(err as Error)?.message
          }`
        );
      }
    } else {
  ...
  private generateUpdateReply(isPremium: boolean, oldEntry: Entry, newEntry: Entry): string {
    ...
    return (
      `\n💲 Buy: ${oldEntry.buy.toValue(keyPrice.metal) !== newEntry.buy.toValue(keyPrice.metal)
        ? `${oldEntry.buy.toString()} → ${newEntry.buy.toString()}`
        : newEntry.buy.toString()
      } | Sell: ${oldEntry.sell.toValue(keyPrice.metal) !== newEntry.sell.toValue(keyPrice.metal)
        ? `${oldEntry.sell.toString()} → ${newEntry.sell.toString()}`
        : newEntry.sell.toString()
      }` +
      `\n📦 Stock: ${amount}` +
      ` | Min: ${oldEntry.min !== newEntry.min ? `${oldEntry.min} → ${newEntry.min}` : newEntry.min} | Max: ${oldEntry.max !== newEntry.max ? `${oldEntry.max} → ${newEntry.max}` : newEntry.max
      }` +
      `\n🛒 Intent: ${oldEntry.intent !== newEntry.intent
        ? `${oldEntry.intent === 2 ? 'bank' : oldEntry.intent === 1 ? 'sell' : 'buy'} → ${newEntry.intent === 2 ? 'bank' : newEntry.intent === 1 ? 'sell' : 'buy'
        }`
        : `${newEntry.intent === 2 ? 'bank' : newEntry.intent === 1 ? 'sell' : 'buy'}`
      }` +
      `\n📋 Enabled: ${oldEntry.enabled !== newEntry.enabled
        ? `${oldEntry.enabled ? '✅' : '❌'} → ${newEntry.enabled ? '✅' : '❌'}`
        : `${newEntry.enabled ? '✅' : '❌'}`
      }` +
      `\n🔄 Autoprice: ${oldEntry.autoprice !== newEntry.autoprice
        ? `${oldEntry.autoprice ? '✅' : '❌'} → ${newEntry.autoprice ? '✅' : '❌'}`
        : `${newEntry.autoprice ? '✅' : '❌'}`
      }` +
      `\n½🔄 isPartialPriced: ${oldEntry.isPartialPriced !== newEntry.isPartialPriced
        ? `${oldEntry.isPartialPriced ? '✅' : '❌'} → ${newEntry.isPartialPriced ? '✅' : '❌'}`
        : `${newEntry.isPartialPriced ? '✅' : '❌'}`
      }` +
      (isPremium
        ? `\n📢 Promoted: ${oldEntry.promoted !== newEntry.promoted
          ? `${oldEntry.promoted === 1 ? '✅' : '❌'} → ${newEntry.promoted === 1 ? '✅' : '❌'}`
          : `${newEntry.promoted === 1 ? '✅' : '❌'}`
        }`
        : '') +
      `\n🔰 Group: ${oldEntry.group !== newEntry.group ? `${oldEntry.group} → ${newEntry.group}` : newEntry.group
      }` +
    ...
    if (params.all === true) {
      ...
      if (params.withgroup || params.withoutgroup || params.withintent) {
        for (const sku in newPricelist) {
          ...
          if (params.withgroup && entity.group === params.withgroup) {
            delete newPricelist[sku];
          } else if (params.withoutgroup && entity.group !== params.withgroup) {
            delete newPricelist[sku];
          } else if (params.withintent && entity.intent === params.withintent) {
            delete newPricelist[sku]
          }
        }
      } else {
      ...
      if (params.withgroup || params.withoutgroup || params.withintent) {
        try {
          ...
          let errMsg = params.withIntent ? `❌ Failed to clear pricelist with/without group: ${(err as Error).message}` : `❌ Failed to clear pricelist with intent: ${(err as Error).message}`
          return this.bot.sendMessage(
            steamID, errMsg
          );
        }
  ...
  async getAllCommand(steamID: SteamID, message: string): Promise<void> {
    ...
    const list = Object.keys(pricelist).map((priceKey, i) => {
      ...
      return `${i + 1}. ${priceKey} - ${name}${name.length > 40 ? '\n' : ' '}(${stock}, ${entry.min}, ${entry.max
        }, ${entry.intent === 2 ? 'bank' : entry.intent === 1 ? 'sell' : 'buy'}, ${entry.enabled ? '✅' : '❌'}, ${entry.autoprice ? '✅' : '❌'
        }, ${entry.group}, ${entry.isPartialPriced ? '✅' : '❌'}${isPremium ? `, ${entry.promoted === 1 ? '✅' : '❌'}` : ''
        })`;
    });
    ...
    this.bot.sendMessage(
      steamID,
      `Found ${pluralize('item', listCount, true)} in your pricelist${limit !== -1 && params.limit === undefined && listCount > 15
  ...
  async partialPriceUpdateCommand(steamID: SteamID, message: string): Promise<void> {
    ...
    const list = Object.keys(pricelist).map((sku, i) => {
      ...
      return `${i + 1}. ${entry.sku} - ${name} (since ${time}), oneScrapDiff? ${isOneScrapDifference ? '✅' : '❌'
        }`;
    });
    ...
    this.bot.sendMessage(
      ...
      `${pluralize('item', listCount, true)} in your pricelist that ${pluralize(
        ...
        listCount,
        true
      )} currently being partial priced${limit !== -1 && params.limit === undefined && listCount > 15
        ? `, showing only ${limit} items (you can send with parameter limit=-1 to list all)`
  ...
  async findCommand(steamID: SteamID, message: string): Promise<void> {
    ...
    const parametersUsed = {
      ...
      intent:
        params.intent !== undefined
          ? `intent=${(params.intent as number) === 0 ? 'buy' : (params.intent as number) === 1 ? 'sell' : 'bank'
          }`
          : '',
      promoted:
    ...
    if (filterCount === 0) {
      ...
      const list = filter.map((entry, i) => {
        ...
        return `${i + 1}. ${priceKey} - ${name}${name.length > 40 ? '\n' : ' '}(${stock}, ${entry.min}, ${entry.max
          ...
          }, ${entry.autoprice ? '✅' : '❌'}, ${entry.group}, ${entry.isPartialPriced ? '✅' : '❌'}${isPremium ? `, ${entry.promoted === 1 ? '✅' : '❌'}` : ''
          })`;
      });
      const listCount = list.length;
      ...
      this.bot.sendMessage(
        steamID,
        `Found ${pluralize('item', filterCount, true)} with ${display.join('&')}${limit !== -1 && params.limit === undefined && listCount > 15
          ? `, showing only ${limit} items (you can send with parameter limit=-1 to list all)`
          : `${limit < listCount && limit > 0 && params.limit !== undefined
            ? ` (limit set to ${limit})`
            : ''
...
class AutoAddQueue {
  ...
  async executeAutoAdd(): Promise<void> {
    ...
    if (this.skusToSkip.includes(this.sku)) {
      ...
      this.bot.sendMessage(
        this.steamID,
        `----------\n⚠️ ${this.bot.schema.getName(SKU.fromString(this.sku))} (${this.sku
