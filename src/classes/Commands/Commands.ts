...
export default class Commands {
    ...
    private async withdrawMptfCommand(steamID: SteamID, message: string): Promise<void> {
        ...
        const withIntent =
            params.withintent == '' || typeof params.withintent !== 'number'
                ? typeof params.withintent === 'string'
        ...
        try {
            ...
            for (const sku in clonedDict) {
                ...
                if (withIntent) {
                    if (withIntent !== this.bot.pricelist.getPrice({ priceKey: sku })?.intent) {
                        delete clonedDict[sku];
                        continue;
                    }
                    isWithinIntent = true;
                }
                if (pureAndWeapons.includes(sku) && !isWithinGroup && !isWithinIntent) {
                    delete clonedDict[sku];
                    continue;
                }
