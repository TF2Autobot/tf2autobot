...
export function uptime(): string {
    ...
    if (hoursDiff >= 21.5 && hoursDiff < 35.5) return `Bot has been up for a day (${hoursDiff} hours).`;
    else if (daysDiff >= 25.5) return `Bot has been up for a month (${daysDiff} days).`;
    else return `Bot has been up for ${uptimeAsMoment.from(currentTime, true)}.`;
}
