...
|            `MINIMUM_KEYS`             | `number`  |   `0`   | When current keys > minimum keys, it will start selling keys (and current ref < minimum ref). Else, it will stop selling keys.                                             |
|            `MAXIMUM_KEYS`             | `number`  |  `10`   | When current keys < maximum keys, it will start buying keys (and current ref > maximum ref). Else, it will stop buying keys.                                               |
| `MINIMUM_REFINED_TO_ START_SELL_KEYS` | `number`  |  `30`   | Already explained.                                                                                                                                                         |
| `MAXIMUM_REFINED_TO_ STOP_SELL_KEYS`  | `number`  |  `150`  | Already explained.                                                                                                                                                         |
...
|       `DISABLE_CRAFTING_ WEAPONS`       | `boolean` | `false` | Set to `true` **if you DO NOT** want your bot to automatically craft any duplicated craftable weapons.                                                 |
...
|   `SKIP_ACCOUNT_ LIMITATIONS`   | `boolean` | `false` | Used to check your account limitation. It's better to set to `true` if your bot's Steam account is already a [premium account](https://support.steampowered.com/kb_article.php?ref=3330-IAGK-7663). |
| `SKIP_UPDATE_ PROFILE_SETTINGS` | `boolean` | `false` | This is just to set your bot's Steam profile to public so backpack.tf can load your bot inventory and etc correctly. If you already set everything to public, just set this to `true`.              |
...
| `X_ADDITIONAL_ DESCRIPTION_NOTE` |  `string`  |  `""`   | Refer example below.                                                                                                                                                                                                                                                                                   |  | `X_MENTION_OWNER` | `boolean` | `false` | Set this to`true`if you want to be mentioned on each successful trade. | `X_MENTION_OWNER* ONLY_ITEMS_SKU` | `string[]` | `[""]` | Support multiple items sku - let say you just want to be mentioned on every unusual and australium trade - just do`[";5;u", ";11;australium"]`. If you want to be mentioned on specific items, just fill in the full item sku like`["725;6;uncraftable"]`. To add more, just separate it with a comma between each sku `string`. |
...
|       `DISABLE_REVIEW_ OFFER_NOTE`       | `boolean`  | `false` | By default, it will show notes on each [reason](https://github.com/idinium96/tf2autobot/blob/master/src/classes/MyHandler.ts#L1355-L1575)                                                                                                                                                                                                                                                                                                               |
...
| `INVALID_VALUE_ EXCEPTION_VALUE_ IN_REF` |  `number`  |   `0`   | - Exception value for the sku(s) that you set above. Default is `0` (no exception).                                                                                                                                                                                                                                                                                                                                                                     |
...
|     `CUSTOM_DECLINED_ MESSAGE`      | `string` |  `""`   | Your custom `DECLINED` note.                                                                                                                                                                                        |
|    `CUSTOM_TRADED_ AWAY_MESSAGE`    | `string` |  `""`   | Your custom note when the bot fails to trade because the item is traded away.                                                                                                                                       |
| `CUSTOM_CLEARING_ FRIENDS_MESSAGE`  | `string` |  `""`   | Your custom note when the bot is removing friends to add someone else.                                                                                                                                              |
